import React from "react";
import styles from "./SurveyGroup.module.css";
import { useNavigate } from "react-router-dom";
import { useSurvey } from "../../context/SurveyContext.jsx";
import { QUESTIONS } from "../../config/questions.js";
import MoneyInput from "../../components/MoneyInput.jsx";
import GroupNavigation from "../../components/GroupNavigation.jsx";
import { formatKRW, formatPercent } from "../../libs/format.js";

/**
 * 부채 페이지 - 부채 관련 문항
 * 문항: 부채 총액, 평균 이자율, 상환 기간, 담보대출 정보
 */
export default function Debt() {
  const navigate = useNavigate();
  const { state, dispatch } = useSurvey();

  // 부채 그룹의 문항들만 필터링
  const debtQuestions = QUESTIONS.filter((q) => q.group === "부채");

  const [errors, setErrors] = React.useState({});

  // 답변 설정 함수
  function setAnswer(id, value) {
    dispatch({ type: "SET_ANSWER", payload: { id, value } });
    // 에러가 있었다면 제거
    if (errors[id]) {
      setErrors((prev) => ({ ...prev, [id]: null }));
    }
  }

  // 입력 검증
  function validate(question, answer) {
    if (question.type === "yesno_multi") {
      if (answer?.has === null) {
        return "예/아니오를 선택해주세요.";
      }
      if (answer?.has === true) {
        for (const field of question.fields) {
          const value = answer[field.key];
          if (value === null || value === "") {
            return `${field.label}을(를) 입력해주세요.`;
          }
          if (
            (field.kind === "money" || field.kind === "number") &&
            value < 0
          ) {
            return `${field.label}은(는) 0 이상이어야 합니다.`;
          }
        }
      }
    }
    return null;
  }

  // 다음 단계로 이동 (결과 페이지)
  function handleNext() {
    let hasError = false;
    const newErrors = {};

    // 모든 문항 검증
    debtQuestions.forEach((q) => {
      const answer = state.answers[q.id];
      const error = validate(q, answer);
      if (error) {
        newErrors[q.id] = error;
        hasError = true;
      }
    });

    if (hasError) {
      setErrors(newErrors);
      return;
    }

    // Mixpanel 이벤트
    console.log("Mixpanel: Debt_Completed");

    // 그룹 완료 상태 업데이트
    dispatch({ type: "COMPLETE_GROUP", payload: { groupName: "부채" } });

    // 결과 페이지로 이동
    navigate("/result");
  }

  // 이전 단계로 이동
  function handlePrev() {
    navigate("/survey/assets");
  }

  // 문항 렌더링
  function renderQuestion(question) {
    const answer = state.answers[question.id] || { has: null };
    const error = errors[question.id];

    return (
      <div key={question.id} className={styles.questionCard}>
        <div className={styles.questionHeader}>
          <h3 className={styles.questionTitle}>{question.title}</h3>
          <div className={styles.questionNumber}>Q{question.no}</div>
        </div>

        <div className={styles.inputArea}>
          <div
            className={styles.yesNoRow}
            role="radiogroup"
            aria-label="예 아니오"
          >
            <label
              className={`${styles.yesNoOption} ${
                answer.has === false ? styles.selected : ""
              }`}
            >
              <input
                type="radio"
                name={`yn-${question.id}`}
                value="no"
                checked={answer.has === false}
                onChange={() => setAnswer(question.id, { has: false })}
              />
              <span>아니오</span>
            </label>
            <label
              className={`${styles.yesNoOption} ${
                answer.has === true ? styles.selected : ""
              }`}
            >
              <input
                type="radio"
                name={`yn-${question.id}`}
                value="yes"
                checked={answer.has === true}
                onChange={() =>
                  setAnswer(question.id, { ...answer, has: true })
                }
              />
              <span>예</span>
            </label>
          </div>

          {answer.has === true && (
            <div className={styles.multiFields}>
              {question.fields.map((field) => (
                <div key={field.key} className={styles.fieldItem}>
                  <label
                    htmlFor={`q-${question.id}-${field.key}`}
                    className={styles.fieldLabel}
                  >
                    {field.label}
                  </label>
                  {field.kind === "money" ? (
                    <MoneyInput
                      id={`q-${question.id}-${field.key}`}
                      value={answer[field.key]}
                      onChange={(value) =>
                        setAnswer(question.id, {
                          ...answer,
                          [field.key]: value,
                        })
                      }
                      rightUnit="만원"
                      placeholder="금액을 입력하세요"
                    />
                  ) : (
                    <div className={styles.numberInput}>
                      <input
                        id={`q-${question.id}-${field.key}`}
                        type="number"
                        inputMode={
                          field.kind === "percent" ? "decimal" : "numeric"
                        }
                        step={field.kind === "percent" ? "0.1" : "1"}
                        min={
                          field.kind === "money" || field.kind === "number"
                            ? 0
                            : undefined
                        }
                        value={answer[field.key] ?? ""}
                        onChange={(e) => {
                          const value =
                            e.target.value === ""
                              ? null
                              : Number(e.target.value);
                          setAnswer(question.id, {
                            ...answer,
                            [field.key]: value,
                          });
                        }}
                        placeholder=""
                        className={styles.input}
                      />
                      <span className={styles.unit}>{field.unit}</span>
                    </div>
                  )}
                </div>
              ))}
            </div>
          )}
        </div>

        {error && (
          <div className={styles.errorMessage} role="alert">
            {error}
          </div>
        )}

        {/* 입력값 미리보기 */}
        {answer.has !== null && (
          <div className={styles.preview}>
            {answer.has === true ? (
              <div className={styles.previewDetails}>
                {question.fields.map((field) => {
                  const value = answer[field.key];
                  if (value === null || value === "") return null;

                  let displayValue;
                  if (field.kind === "money") {
                    displayValue = formatKRW(value);
                  } else if (field.kind === "percent") {
                    displayValue = formatPercent(value);
                  } else {
                    displayValue = `${value}${field.unit}`;
                  }

                  return (
                    <div key={field.key} className={styles.previewItem}>
                      {field.label}: {displayValue}
                    </div>
                  );
                })}
              </div>
            ) : (
              "없음"
            )}
          </div>
        )}
      </div>
    );
  }

  return (
    <div className={`container ${styles.page}`}>
      <div className={styles.header}>
        <button
          className={styles.backButton}
          onClick={handlePrev}
          aria-label="이전으로"
        >
          ←
        </button>
        <h1 className={styles.pageTitle}>부채</h1>
        <div className={styles.progress}>
          <span className={styles.currentStep}>5</span>
          <span className={styles.totalSteps}>/ 5</span>
        </div>
      </div>

      <div className={styles.content}>
        <GroupNavigation />

        <div className={styles.groupDescription}>
          <p>현재 보유하고 있는 부채 정보를 입력해주세요.</p>
        </div>

        <div className={styles.questionsContainer}>
          {debtQuestions.map(renderQuestion)}
        </div>
      </div>

      <div className={styles.footer}>
        <button className={styles.nextButton} onClick={handleNext}>
          진단 결과 보기
        </button>
      </div>
    </div>
  );
}
